#!/bin/sh
cd /src/builder || exit
grep -E 'CONFIG_PACKAGE.+(kmod|firmware)' /src/builder/.config | grep -E "is not set|=" | grep -Eo "CONFIG_PACKAGE[-_a-zA-Z0-9]+" | sed "s/CONFIG_PACKAGE_//g" | grep -Ev "qemu|-usb|kmod-input|-raid|kmod-leds|bigclown|bluetooth|wireless|wifi|iwl|sound|video|pcmcia|gpio|kmod-ipt-|kmod-nf-|kmod-nft-|kmod-fs-|ath10k-|brcmsmac|brcmfmac|-sdio|openvswitch|-ath" | sort -u >/src/builder/allmod.list
