#!/bin/sh
cd /src/builder || exit
grep -E 'CONFIG_PACKAGE.+(kmod|firmware)' /src/builder/.config | grep -E "is not set|=" | grep -Eo "CONFIG_PACKAGE[-_a-zA-Z0-9]+" | sed "s/CONFIG_PACKAGE_//g" | grep -Ev "kmod-usb|kmod-input|-raid|kmod-leds|qemu|bigclown|bluetooth|wireless|wifi|sound|video|pcmcia|gpio|kmod-ipt-|kmod-nf-|kmod-nft-|kmod-fs-|ath10k-|brcmsmac|brcmfmac|-sdio|openvswitch" | sort -u >/src/builder/allmod.list
